...
import { requestMagicLink, signUp } from './actions';
...
export function Login({ mode = 'signin' }: { mode?: 'signin' | 'signup' }) {
  ...
  const [state, formAction, pending] = useActionState<ActionState, FormData>(
    mode === 'signin' ? requestMagicLink : signUp,
    { error: '' }
  );
  ...
  const formIsValid =
    mode === 'signin' ? emailIsValid : emailIsValid && passwordInput.length >= 8;

  return (
    <div className="flex min-h-[100dvh] w-full bg-[var(--background)] text-[var(--foreground)]">
      <section className="flex w-[420px] shrink-0 flex-col gap-5 border-r border-[var(--border)] bg-[#000] pl-12 pr-6 py-8 text-left">
        <div>
          <div className="flex justify-center mb-3">
            <img src="/logo.png" alt="LiveKit" style={{ width: 120, height: 'auto' }} />
          </div>
          <div className="h-px w-full bg-[var(--border)]" />
          <h1 className="mt-4 text-[18px] font-semibold leading-[28px] tracking-normal text-[var(--foreground)]">
        ...
        <div className="space-y-2">
          {oauthActions.map((action) => (
            <button
              ...
              className="flex w-full items-center gap-3 rounded border border-[var(--border)] bg-transparent px-6 py-2.5 text-left text-[var(--foreground)] transition duration-[var(--transition-default)] hover:border-[var(--accent-cyan-bright)] hover:text-[var(--accent-cyan-bright)]"
            >
        ...
        <p className="text-sm text-[var(--foreground-muted)]">
          {mode === 'signin'
            ? 'Request a link to sign in or sign up'
        ...
        <form action={formAction} className="space-y-3">
          ...
          <button
            ...
            className="w-full rounded-full bg-[#00e5ff] px-4 py-3 text-sm font-semibold text-[#000] shadow-[0_0_12px_rgba(0,229,255,0.6)] transition duration-[var(--transition-default)] disabled:cursor-not-allowed disabled:bg-[#00e5ff]/40 disabled:shadow-none disabled:text-[#000]/50"
          >
            {pending ? (
              <>
      ...
      <section className="relative flex-1 sticky top-0 h-[100dvh] bg-[var(--background)]">
        <img
          src="/world_map.svg"
          alt="World Map"
